...
; Don't call tex directly - the latex->pict needs
; either "textmath" or "displaymath" to extract the contents.
; (There are also options that can extract titles and footnotes,
;  but not plain text.)

(define
  (tex x
       #:document-class-options [doc-options (or (current-document-class-options) '())]
       #:scale    [s               (or (current-scale-factor) 1)]
  ...
  (define preview-opts
    (match preview-options
      [(? string? s) (list s)]
      [(? list? xs)  xs]
      [_ (error 'tex (~a "got: " preview-options))]))

  ;(display x)

  (define p (latex->pict x
                         #:document-class-options doc-options
                         #:preview-options        preview-opts
                         #:preamble               preamble
                         #:extract-baseline-fraction? #t))
  ; (def f      (or last-extracted-baseline-fraction 1.1))  
  ; (defv (w h) (values (pict-width p) (pict-height p)))
  (scale p s))

;;; Specialized math versions

(define (tex-math
         x
         #:document-class-options [doc-options     (or (current-document-class-options) '())]
         #:preview-options        [preview-options (or (current-preview-options)        '())]
         #:preamble               [preamble        (or (current-preamble)               "")]
         #:scale                  [s               (or (current-scale-factor)           1)])
  
  (tex (~a "\\(" x "\\)")
       #:document-class-options doc-options
       #:preview  (cons "textmath" preview-options)
       #:scale s
       #:preamble preamble))
...
(define (mathpar
    ...
    (tex (~a "\\begin{mathpar}" x "\\end{mathpar}")
       #:document-class-options doc-options
       #:preview  (cons "textmath" preview-options)
       #:scale s
       #:preamble preamble))

(define (tex-display-math x
         #:document-class-options [doc-options     (or (current-document-class-options) '())]
         #:preview-options        [preview-options (or (current-preview-options)        '())]
         #:preamble               [preamble        (or (current-preamble)               "")]
         #:scale                  [s               (or (current-scale-factor)           1)])
  ; ...
  (tex (~a "\\(\\displaystyle " x " \\)")
       #:document-class-options doc-options
       #:preview (cons "textmath" preview-options)
       #:scale s
       #:preamble preamble))

(define (tex-real-display-math x
         #:document-class-options [doc-options     (or (current-document-class-options) '())]
         #:preview-options        [preview-options (or (current-preview-options)        '())]
         #:preamble               [preamble        (or (current-preamble)               "")]
         #:scale                  [s               (or (current-scale-factor)           1)])
  ...
  (tex (~a "\\[ " x " \\]")
       #:document-class-options doc-options
       #:preview (cons "displaymath" preview-options)
       #:scale s
       #:preamble preamble))
